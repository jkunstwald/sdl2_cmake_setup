...
Source: "data\sdl2-config.cmake"; DestDir: "{app}\SDL2-2.0.18"; Flags: ignoreversion

[Dirs]
; Create the include/SDL2 folder to later on move headers into
Name: "{app}\SDL2-2.0.18\include\SDL2"
...
Root: HKLM; Subkey: "System\CurrentControlSet\Control\Session Manager\Environment"; ValueType:string; ValueName: "SDL2_DIR"; \
    ValueData: "{app}\SDL2-2.0.18"; Flags: preservestringtype uninsdeletevalue

[UninstallDelete]
Type: filesandordirs; Name: "{app}\SDL2-2.0.18"

[Code]
procedure InitializeWizard();
begin
    idpAddFile('https://www.libsdl.org/release/SDL2-devel-2.0.18-VC.zip', ExpandConstant('{tmp}\SDL2.zip'));
...
begin
    ...
    begin
        ...
        Exec('cmd.exe', ExpandConstant('/c MOVE "{app}\SDL2-2.0.18\include\*" "{app}\SDL2-2.0.18\include\SDL2\"'), '', SW_HIDE, ewWaitUntilTerminated, ErrorCode);
    end;
end;
